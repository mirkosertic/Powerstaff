...
public class TitleRenderer extends BaseRenderer {

    @Override
    public void encodeEnd(FacesContext aContext, UIComponent aComponent) throws IOException {
        TitleComponent theComponent = (TitleComponent) aComponent;

        String theTitle = theComponent.getTitle();

        ResponseWriter theWriter = aContext.getResponseWriter();

        theWriter.startElement("script", theComponent);
        theWriter.writeAttribute("type", "text/javascript", null);
        theWriter.append("setTitle('" + theTitle + "');");
