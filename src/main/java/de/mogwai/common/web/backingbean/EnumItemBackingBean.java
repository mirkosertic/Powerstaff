 ...
 * @version $Date: 2008-07-12 14:10:00 $
 */
public abstract class EnumItemBackingBean extends BackingBean {
     ...
     * Kategorie. Map-Key ist <code>EnumItemCategory.id</code>. Sortierung
     * nach deutschen Labels.
     */
    ...
    /**
     * Gibt den Wert des Attributs <code>enumItemCategoryService</code>
     * zurück.
     * 
     ...
     */
    public void setEnumItemCategoryService(EnumItemCategoryService enumItemCategoryService) {
    ...
    public EnumItem getEnumItem(long id) {
        EnumItem enumItem = this.getEnumItemMap().get(new Long(id));
        if (enumItem == null) {
            String msg = MessageFormat.format("Es existiert kein EnumItem mit ID {0}.", id);
     ...
     *         definiert.
     */
    public EnumItem getDefaultEnumItem(BaseEnumItemCategoryEnum enumItemCategoryEnum) {

        Long defaultEnumItemId = this.getDefaultEnumItemMap().get(enumItemCategoryEnum.getId());
     ...
     */
    public EnumItemMultiLanguageSelectItemList getSelectItemList(BaseEnumItemCategoryEnum enumItemCategoryEnum) {
        ...
        if (Locale.FRENCH.equals(currentLocale)) {
            if (this.selectItemCategoryMapFR == null) {
                this.selectItemCategoryMapFR = this.getSelectItemCategoryMap(currentLocale);
            }
            return new EnumItemMultiLanguageSelectItemList(this.selectItemCategoryMapFR.get(enumItemCategoryEnum
                    .getId()));
        }

        if (this.selectItemCategoryMapDE == null) {
            this.selectItemCategoryMapDE = this.getSelectItemCategoryMap(currentLocale);
        }
        return new EnumItemMultiLanguageSelectItemList(this.selectItemCategoryMapDE.get(enumItemCategoryEnum.getId()));
    }
     ...
     */
    private Map<Long, EnumItemMultiLanguageSelectItemList> getSelectItemCategoryMap(Locale locale) {

        Map<Long, EnumItemMultiLanguageSelectItemList> selectItemCategoryMap = new HashMap<Long, EnumItemMultiLanguageSelectItemList>();
        List<EnumItemCategory> enumItemCategoryList = this.enumItemCategoryService.getEnumItemCategoryList();

        for (Iterator iter = enumItemCategoryList.iterator(); iter.hasNext();) {
            EnumItemCategory enumItemCategory = (EnumItemCategory) iter.next();

            List<EnumItem> enumItemsOfOneCategory = new LinkedList<EnumItem>(enumItemCategory.getEnumItems());
            ...
            if (enumItemCategory.getIsAlphabeticalOrder().booleanValue()) {
                ...
                Collections.sort(enumItemsOfOneCategory, new EnumItemLabelComparator(locale));
            } else {
                // Sortierung nach listIndex
                Collections.sort(enumItemsOfOneCategory, new EnumItemListIndexComparator());
            }

            for (EnumItem enumItem : enumItemsOfOneCategory) {
                EnumItemMultiLanguageSelectItem selectItem = new EnumItemMultiLanguageSelectItem(enumItem);
        ...
        if (this.defaultEnumItemMap == null) {
            ...
            for (Iterator iter = enumItemCategoryList.iterator(); iter.hasNext();) {
                ...
                if (((theItem != null) && (theItem.getId() != null))) {
                    this.defaultEnumItemMap.put(enumItemCategory.getId(), theItem.getId());
                }
            }
        }
