...
public class WebsiteURLDAOImpl implements WebsiteDAO {
	...
	private final static Logger LOGGER=new Logger(WebsiteURLDAOImpl.class);
	...
	protected InputStream executeStatement(String aStatement,HashMap aParams) throws IOException {
		...
		LOGGER.logDebug("Response code was "+theConnection.getResponseCode()+" "+theConnection.getResponseMessage());
	...
	protected void executeWithoutResult(String aStatement,HashMap aParams) {
		...
		try {
			...
			LOGGER.logDebug("Server returned error string "+theResult.getError()+" for statement "+theResult.getStatement());			
	...
	public WebProject findByOriginId(Long aId) {
		...
		String theStatement="SELECT id,date,projectNumber,workplace,start,duration,descriptionShort,descriptionLong,originId,ASCII(notifiedForNewsletter) as notified FROM webproject WHERE originId="+aId;
