...
public class Datenuebernahme {
	...
	public static void main(String[] args) throws InstantiationException,
			ClassNotFoundException, SQLException, NotSupportedException,
		...
		String theJDBCURL = "jdbc:odbc:Driver={Microsoft Access Driver (*.mdb)};;DBQ="
				+ theFileName;
		...
		ApplicationContext theContext = new ClassPathXmlApplicationContext(
				"applicationContext-uebernahme.xml");
		final SessionFactory theFactory = (SessionFactory) theContext
				.getBean("sessionFactory");

		PlatformTransactionManager theManager = (PlatformTransactionManager) theContext
				.getBean("txManager");

		File theCVPath = new File(
				"C:\\Daten\\Arbeit\\Projekte\\MobileConsulting\\CVPath");
	...
	private static boolean getBoolean(ResultSet aResultSet, String aFieldName)
			throws SQLException {
		String theValue = getString(aResultSet, aFieldName);
		return "1".equals(theValue);
	}

	private static Timestamp getTimestamp(ResultSet aResultSet,
			String aFieldName) throws SQLException {
		return aResultSet.getTimestamp(aFieldName);
	}

	private static Date getDate(ResultSet aResultSet, String aFieldName)
			throws SQLException {
		return aResultSet.getDate(aFieldName);
	}

	private static Long getLong(ResultSet aResultSet, String aFieldName)
			throws SQLException {
	...
	private static String getLongString(ResultSet aResultSet, String aFieldName)
			throws SQLException {
		Long theValue = aResultSet.getLong(aFieldName);
		if (theValue == null) {
			return null;
	...
	private static String getString(ResultSet aResultSet, String aFieldName)
			throws SQLException {
	...
	private static String getConcatenatedString(ResultSet aResultSet,
			String... aFieldName) throws SQLException {
	...
	private static void resultSetToUDF(ResultSet aResult, UDFSupport aObject)
			throws SQLException {
	...
	private static void importMitarbeiter(final SessionFactory aFactory,
			final PlatformTransactionManager aManager, Connection aConnection,
			File aCVPath) throws SQLException, IOException {
		...
		ResultSet thePersonalResult = theFindPersonalStatenebt
				.executeQuery("select * from personal");
		while (thePersonalResult.next()) {
			thePersonalMap.put(thePersonalResult.getString("ID"),
					thePersonalResult.getString("loginName"));
		}
		...
		while (theMitarbeiterResult.next()) {
			...
			LOGGER.logInfo("Verarbeite " + theCounter + "-> "
					+ theMitarbeiterID);

			// Freiberufler
			final Freelancer theFreelancer = new Freelancer();
			theFreelancer.setCreationDate(getTimestamp(theMitarbeiterResult,
					"cvEing"));
			theFreelancer.setCreationUserID(thePersonalMap.get(getString(
					theMitarbeiterResult, "erstPersID")));
			theFreelancer.setLastModificationDate(getTimestamp(
					theMitarbeiterResult, "modifdatum"));
			theFreelancer.setLastModificationUserID(thePersonalMap
					.get(getString(theMitarbeiterResult, "modifPersID")));

			theFreelancer.setName1(getString(theMitarbeiterResult, "name"));
			theFreelancer.setName2(getString(theMitarbeiterResult, "vorname"));
			theFreelancer.setContactforbidden(getBoolean(theMitarbeiterResult,
					"passiv"));
			theFreelancer.setComments(getConcatenatedString(
					theMitarbeiterResult, "notiz", "notiz2", "bewertung"));

			theFreelancer.setWorkplace(getConcatenatedString(
					theMitarbeiterResult, "wunschort", "wunschlandPLZ"));
			theFreelancer.setAvailabilityAsDate(getDate(theMitarbeiterResult,
					"verfvon"));
			theFreelancer.setSallaryLong(getLong(theMitarbeiterResult, "satz"));
			theFreelancer.setCode(getString(theMitarbeiterResult, "cvNr"));
			theFreelancer
					.setSkills(getString(theMitarbeiterResult, "kurzskill"));
			theFreelancer.setGulpID(getLongString(theMitarbeiterResult,
					"gulpId"));
			theFreelancer.setGeburtsdatum(getString(theMitarbeiterResult,
					"gebDatum"));
			...
			if (theStatus > 0) {
				theFreelancer.setStatus(Integer.valueOf(theStatus));
			}

			ResultSet theAdresseResult = theAdresse
					.executeQuery("select * from mitarbeiter_adresse where mitArbID = "
							+ theMitarbeiterID);
			while (theAdresseResult.next()) {

				boolean theMaster = getBoolean(theAdresseResult, "master");
				if (theMaster) {
					theFreelancer.setCompany(getString(theAdresseResult,
							"firma"));
					theFreelancer.setStreet(getString(theAdresseResult,
							"strasse"));
					theFreelancer
							.setCountry(getString(theAdresseResult, "land"));
				...
				if (!(theValue == null)) {
					...
					theContact.setType(theMaster ? theTelGesContactType
							: theTelPrivatContactType);
				...
				if (!(theValue == null)) {
					FreelancerContact theContact = new FreelancerContact();
					theContact.setType(theMaster ? theMobileGesContactType
							: theMobilePrivatContactType);
					theContact.setValue(theValue);
					theFreelancer.getContacts().add(theContact);
				}
				...
				if (!(theValue == null)) {
					FreelancerContact theContact = new FreelancerContact();
					theContact.setType(theMaster ? theFaxGesContactType
							: theFaxPrivatContactType);
					theContact.setValue(theValue);
					theFreelancer.getContacts().add(theContact);
				}
				...
				if (!(theValue == null)) {
					FreelancerContact theContact = new FreelancerContact();
					theContact.setType(theMaster ? theMailGesContactType
							: theMailPrivatContactType);
					theContact.setValue(theValue);
					theFreelancer.getContacts().add(theContact);
				}
				...
				if (!(theValue == null)) {
					FreelancerContact theContact = new FreelancerContact();
					theContact.setType(theMaster ? theWebGesContactType
							: theMailPrivatContactType);
					theContact.setValue(theValue);
					theFreelancer.getContacts().add(theContact);
				}
			 ...
			 */
			ResultSet theReadAgain = theReadAgainStatement
			...
			if (!StringUtils.isEmpty(theFreelancer.getCode())) {
				if (!StringUtils.isEmpty(theField.getLongStringValue())) {
					File theCV = new File(aCVPath, "Profil "
							+ theFreelancer.getCode() + ".txt");
					...
					if (theFreelancer.getCreationDate() != null) {
						theCV.setLastModified(theFreelancer.getCreationDate()
								.getTime());
					}

					ResultSet theCVAktu = theReadAgainStatement
							.executeQuery("select * from aktivitat where cvnr = '"
									+ theFreelancer.getCode()
									+ "' and aktivitat = 'CV aktualisiert' order by datum desc");
					if (theCVAktu.next()) {
						theCV.setLastModified(getDate(theCVAktu, "datum")
								.getTime());
					}
					theCVAktu.close();
				}
	...
	private static void importKunden(final SessionFactory aFactory,
			final PlatformTransactionManager aManager, Connection aConnection)
		...
		ResultSet theFindLandResult = theFindLandStatenebt
				.executeQuery("select * from land");
		while (theFindLandResult.next()) {
			theLandMap.put(theFindLandResult.getInt("ID"), theFindLandResult
					.getString("landKuerzel"));
		}
		...
		ResultSet theKundenResult = theKundenSelect
				.executeQuery("select * from kunde");
		long theCounter = 0;
		while (theKundenResult.next()) {
			...
			String theKundenId = theRootCustomer.getUdf().get("kunde")
					.getStringValue();

			LOGGER.logInfo("Verarbeite " + theCounter + "-> " + theKundenId);

			ResultSet theKontakte = theAdresse
					.executeQuery("select * from ansprechpartner where kunde = '"
							+ theKundenId + "'");
			while (theKontakte.next()) {
				...
				theCustomer.setCompany(theRootCustomer.getUdf().get("firma")
						.getStringValue());
				theCustomer.setStreet(theRootCustomer.getUdf().get("strasse")
						.getStringValue());
				theCustomer.setPlz(theRootCustomer.getUdf().get("plz")
						.getStringValue());
				theCustomer.setCity(theRootCustomer.getUdf().get("ort")
						.getStringValue());

				UserDefinedField theLandField = theRootCustomer.getUdf().get(
						"landID");
				if (theLandField != null) {
					theCustomer.setCountry(theLandMap.get(theLandField
							.getIntValue()));
				}

				String theValue = theRootCustomer.getUdf().get("tel1")
						.getStringValue();
				if (!(theValue == null)) {
				...
				theValue = theRootCustomer.getUdf().get("tel2")
						.getStringValue();
				if (!(theValue == null)) {
					CustomerContact theContact = new CustomerContact();
					theContact.setType(theTelContactType);
				...
				theValue = theRootCustomer.getUdf().get("email")
						.getStringValue();
				if (!(theValue == null)) {
					CustomerContact theContact = new CustomerContact();
				...
				theCustomer.setCreationDate(getTimestamp(theKontakte,
						"erstdatum"));
				theCustomer.setCreationUserID(thePersonalMap.get(getString(
						theKontakte, "erstPersID")));
				theCustomer.setLastModificationDate(getTimestamp(theKontakte,
						"modifdatum"));
				theCustomer.setLastModificationUserID(thePersonalMap
						.get(getString(theKontakte, "modifPersID")));

				ResultSet theReadAgain = theReadAgainStatement
						.executeQuery("select * from ansprechpartner where ID = "
								+ theId);
				theReadAgain.next();
				...
				ResultSet theHistoryResult = theHistoryStatement
						.executeQuery("select * from ansp_kontakte where anspID = "
								+ theId);
				while (theHistoryResult.next()) {
					...
					theHistory.setCreationUserID(getString(theHistoryResult,
							"person"));
					theHistory
							.setDescription(getString(theHistoryResult, "typ")
									+ "\n"
									+ getString(theHistoryResult, "notiz"));
				...
				theDefinition
						.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRED);
				TransactionStatus theTransaction = aManager
						.getTransaction(theDefinition);
	...
	private static void importPartner(final SessionFactory aFactory,
			final PlatformTransactionManager aManager, Connection aConnection)
			throws SQLException, IOException {
		...
		ResultSet theKundenResult = theKundenSelect
				.executeQuery("select * from kunde where agentur = 0");
		long theCounter = 0;
		while (theKundenResult.next()) {
			theCounter++;
			...
			while (theKontakte.next()) {
				...
				// Wurzeldaten kopieren
				thePartner.setCompany(theRootCustomer.getUdf().get("firma")
						.getStringValue());
				thePartner.setStreet(theRootCustomer.getUdf().get("strasse")
						.getStringValue());
				thePartner.setPlz(theRootCustomer.getUdf().get("plz")
						.getStringValue());
				thePartner.setCity(theRootCustomer.getUdf().get("ort")
						.getStringValue());

				UserDefinedField theLandField = theRootCustomer.getUdf().get(
						"landID");
				if (theLandField != null) {
					thePartner.setCountry(theLandMap.get(theLandField
							.getIntValue()));
				}
				...
				thePartner.setCreationDate(getTimestamp(theKontakte,
						"erstdatum"));
				thePartner.setCreationUserID(thePersonalMap.get(getString(
						theKontakte, "erstPersID")));
				thePartner.setLastModificationDate(getTimestamp(theKontakte,
						"modifdatum"));
				thePartner.setLastModificationUserID(thePersonalMap
						.get(getString(theKontakte, "modifPersID")));
	...
	private static void importProjekte(final SessionFactory aFactory,
			final PlatformTransactionManager aManager, Connection aConnection)
			throws SQLException, IOException {
		...
		while (theProjectResult.next()) {
			...
			theProject.setWorkplace(getConcatenatedString(theProjectResult,
					"land", "plz", "ort"));
			theProject.setStartDate(getDate(theProjectResult, "startDatum"));
			theProject.setDuration(getConcatenatedString(theProjectResult,
					"dauer", "dauerEinheit"));
			theProject.setDescriptionShort(getString(theProjectResult,
					"prTitel"));
			theProject
					.setDescriptionLong(getString(theProjectResult, "aufgabe"));
			...
			theProject.setCreationDate(getTimestamp(theProjectResult,
					"erstdatum"));
			theProject.setCreationUserID(thePersonalMap.get(getString(
					theProjectResult, "erstPersID")));
			theProject.setLastModificationDate(getTimestamp(theProjectResult,
					"modifdatum"));
			theProject.setLastModificationUserID(thePersonalMap.get(getString(
					theProjectResult, "modifPersID")));

			ResultSet theReadAgain = theReadAgainStatement
					.executeQuery("select * from projekt where ID = "
							+ theProjectId);
			theReadAgain.next();
			...
			try {
				if (thePLID != null) {
					...
					String theKunde = theProject.getUdf().get("kunde")
							.getStringValue();
					Customer theDummyCustomer = theDummies.get(theKunde);
					if (theDummyCustomer == null) {

						Statement theCustomerStatement = aConnection
								.createStatement();
						ResultSet theCustomerResult = theCustomerStatement
								.executeQuery("select * from kunde where kunde = '"
										+ theKunde + "'");
						while (theCustomerResult.next()) {
							...
							theDummyCustomer.setCompany(getString(
									theCustomerResult, "firma"));
							theDummyCustomer.setStreet(getString(
									theCustomerResult, "strasse"));
							theDummyCustomer.setPlz(getString(
									theCustomerResult, "plz"));
							theDummyCustomer.setCity(getString(
									theCustomerResult, "ort"));

							String theLandField = getString(theCustomerResult,
									"landID");
							if (theLandField != null) {
								theDummyCustomer.setCountry(theLandMap
										.get(Integer.parseInt(theLandField)));
							}

							String theValue = getString(theCustomerResult,
									"tel1");
							if (!(theValue == null)) {
								CustomerContact theContact = new CustomerContact();
								theContact.setType(theTelContactType);
